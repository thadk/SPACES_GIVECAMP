          ...
          requestType:(MGTwitterRequestType)requestType responseType:(MGTwitterResponseType)responseType
{
    if (self = [super initWithRequest:request delegate:delegate]) {
        ...
        _identifier = [[NSString stringWithNewUUIDThatIsntApples] retain];
...
- (NSString *)description
{
    ...
    return [description stringByAppendingFormat:@" (requestType = %d, identifier = %@, data = %@)", _requestType, _identifier, _data ? [[[NSString alloc] initWithData:_data encoding:NSUTF8StringEncoding] autorelease] : @"<>"];
}
