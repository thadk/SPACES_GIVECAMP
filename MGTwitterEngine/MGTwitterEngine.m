...
#if DEBUG
    if (YES) {
		CFShow([NSString stringWithFormat:@"MGTwitterEngine: finalURL = %@", finalURL]);
	}
#endif
...
//alex: made the sinceID param be unsigned long long because it used to be just unsigned long
...
{
	return [self getSearchResultsForQuery:query sinceID:sinceID maxID:0 startingAtPage:page count:count geocode:geocode]; // zero means default

}

- (NSString *)getSearchResultsForQuery:(NSString *)query sinceID:(unsigned long long)sinceID maxID:(unsigned long long)maxID startingAtPage:(int)page count:(int)count geocode:(NSString *)geocode
{
